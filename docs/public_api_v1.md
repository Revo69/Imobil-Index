...
| `refreshed_at` | Timestamp when the public API row was refreshed from the Gold layer. |
...
|---|---|
| `refresh_gold_estate()` | `api_estate_current`, `api_estate_daily`, `api_rent_yield` |
...
`api_rent_current`, and `api_rent_daily` should have the same latest `date` as
their matching Gold source.
