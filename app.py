...
with tab_rent_daily:
    ...
    if not df_yield.empty:
        ...
        # Сравнение — актуальные данные декабря 2025
        st.markdown("---")
        st.markdown("<h2 style='text-align:center;'>Посуточная vs Помесячная аренда (декабрь 2025)</h2>", unsafe_allow_html=True)
        
        col_a, col_b = st.columns(2)
        with col_a:
            st.markdown("**Помесячная аренда**")
            st.markdown("• 400–600 €/мес за квартиру\n"
                        "• Доходность: **6.9–9.1% годовых**\n"
                        "• Стабильно, низкие расходы")
        
        with col_b:
            st.markdown("**Посуточная аренда** (60% загрузка)")
            st.markdown("• 1 000–1 350 €/мес за ту же квартиру\n"
                        "• Доходность: **8.9–23.3% годовых**\n"
                        "• В 1.9 раза выше помесячной")
        
        st.markdown(
            "<div style='text-align:center; font-size:1.4em; margin:2em; color:#888;'>"
            "Посуточная аренда приносит <b>в 1.9 раза больше денег</b> при тех же вложениях"
            "</div>",
            unsafe_allow_html=True
        )
        
        st.info("Лидер — Чокана: 23.3% годовых при посуточной аренде.\n"
                "БАМ: 8.9% — стабильный фаворит для инвестиций.")
